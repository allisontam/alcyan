...
max_nodes = 30
train_size = 9000
params = kohonen.kohonen.Parameters(dimension=400, shape=(10,10))
num_epochs = 5
train_run = True
...
### TRAIN MODEL
if train_run:
    som = kohonen.kohonen.Map(params) # initialization
    for epoch in range(num_epochs):
        for cue in trainset:
            som.learn(cue)
        np.random.shuffle(trainset)

    pickle.dump(som, open('som-30.pkl','wb'))

### TRANSFORM MODEL INTO CLASSIFIER
if not train_run:
    with open('som-30.pkl','rb') as f:
        som = pickle.load(f)

img = som.neuron_heatmap()
img.save('test.png')
...
for entry in data[:train_size]:
    truth = int(entry[0])
    cue = entry[1:]
    cluster_rep[som.winner(cue)].append(truth)
