...
class SoniaFunc(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        do_dw = A*tanh_grad
        do_dw[int(hidden_num):, :] = 0 # don't want to affect masked convolution weights (2)
        grad_weight = grad_output.mm(do_dw) # not element multiplication?

        do_dx = -A*tanh_grad
        do_dx[int(hidden_num):, :] = 0 # don't want to affect masked convolution weights (3)
        grad_input = grad_output.mm(do_dx) # not element multiplication?
